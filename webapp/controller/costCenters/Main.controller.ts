...
/**
 * @namespace siagrob1.controller.costCenters
 */
export default class Main extends BaseController {
  formatter = { ...formatter }

	onInit(): void {
		this.getRouter().getRoute("costCenters").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void {
		(this.getView().byId("costCentersTable").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void {
		...
		(this.getView().byId("costCentersTable").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
		this.navTo("costCentersNew");
	}

	onEdit(): void {
		const oTable = this.byId("costCentersTable") as Table;
    ...
    if (i < 0){
      ...
      return;
		...
		this.navTo("costCentersEdit", {id: sId});
	}
