...
export default class SelectShipmentRelease extends BaseController {
  ...
  onInit(): void {
    ...
    this.getRouter()
      .getRoute("storageEntryTransactionSelectRelease")
  ...
  onCreateStorageEntry() {
    const oTable = this.byId("tableSelectShipmentReleaseForEntry") as List;
    ...
    this.navTo("storageEntryTransactionCreate", {
      "?query": { shipmentReleaseKey: key }
    });
  }

  onCancel() {
    // Volta para a seleção de armazém preservando o produto já escolhido.
    if (this._itemCode) {
