...
/**
 * @namespace siagrob1.controller.agents
 */
export default class Main extends BaseController {
  formatter = { ...formatter }
  
	onInit(): void | undefined {
		this.getRouter().getRoute("agents").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void | undefined {
		(this.getView().byId("tableAgents").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void | undefined {
		...
		(this.getView().byId("tableAgents").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
			this.navTo("agentsNew");
	}

	onEdit(): void {
		const oTable = this.byId("tableAgents") as Table;
