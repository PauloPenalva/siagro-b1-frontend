...
/**
 * Buffer JSON do diálogo de Vincular Saída do Lote (GAC-1181 fase 2, Task 10). Diálogo à parte
 * do `TransshipmentDialogForm` acima: a action `ShipmentLoadsTransshipmentAttachLotExit` só tem
 * `Key` e `LotExitStorageTransactionKey` — nenhum campo de data — e reaproveitar o diálogo de
 * Iniciar/Registrar Entrada arrastaria pra cá a validação de data que aquele sempre exige.
 */
type TransshipmentLotExitForm = {
  title: string;
  /** Chave do TRANSBORDO (não do romaneio de saída). */
  key: string;
  lotExitKey?: string;
  lotExits: TransshipmentReceiptOption[];
};
...
export abstract class BaseController extends CommonController {
  ...
  private _transshipmentLotExitDialog: Dialog;

  /** Trava ANTES do primeiro await: duplo clique vincularia a saída do lote duas vezes. */
  private _transshipmentLotExitInFlight = false;
  ...
  /**
   * Romaneios `Shipment (1)` elegíveis para vincular como a saída do LOTE (GAC-1181 fase 2,
   * Task 10) — mesmas condições de `ShipmentLoadTransshipmentRules.EnsureLotExitIsUsable` no
   * servidor: confirmado, do MESMO lote da entrada, e ainda sem vínculo com outra carga ou outro
   * transbordo. Espelho de `loadEligibleTransshipmentReceiptsAsync`, com o tipo de romaneio e o
   * filtro de lote trocados.
   *
   * Filtro de ENUM como string crua no `$filter`, nunca `new Filter(...)`: o UI5 não sabe
   * formatar o literal de um enum a partir do metadata e estoura "Unsupported type".
   */
  private async loadEligibleLotExitOptionsAsync(
    storageAddressCode: string
  ): Promise<TransshipmentReceiptOption[]> {
    // Escapa aspas simples no literal do $filter — mesma convenção de
    // Attach.controller#applyShipmentFilters e loadEligibleTransshipmentReceiptsAsync acima.
    const escape = (value: string) => (value ?? "").replace(/'/g, "''");

    const filter = [
      "TransactionType eq 'Shipment'",
      "TransactionStatus eq 'Confirmed'",
      `StorageAddressCode eq '${escape(storageAddressCode)}'`,
      "ShipmentLoadKey eq null",
      "ShipmentLoadTransshipmentKey eq null",
    ].join(" and ");
  ...
  /**
   * Vincular Saída do Lote (GAC-1181 fase 2, Task 10): abre o diálogo que lista os `Shipment (1)`
   * confirmados do lote deste transbordo, ainda sem vínculo. É este vínculo que EMITE a liberação
   * pela quantidade REAL carregada — `ShipmentLoadsTransshipmentAttachLotExitService` no servidor.
   *
   * As duas travas espelham as regras do servidor (`EnsureEntryIsRegistered` /
   * `EnsureLotExitNotAlreadyAttached`), na mesma forma de alerta já usada em
   * `onRegisterTransshipmentEntry`: o botão fica visível por todo o `InTransshipment` (mesma
   * convenção dos vizinhos), então quem decide se o transbordo SELECIONADO está no estado certo é
   * o clique, não a visibilidade do botão.
   */
  async onAttachTransshipmentLotExit(): Promise<void> {
    const context = this.selectedTransshipmentContext();

    if (!context) return;

    const entryStorageTransactionKey = context.getProperty("EntryStorageTransactionKey") as string;

    if (!entryStorageTransactionKey) {
      MessageBox.alert("Registre a entrada do transbordo antes de vincular a saída do lote.");
      return;
    }

    if (context.getProperty("LotExitStorageTransactionKey")) {
      MessageBox.alert("Este transbordo já tem uma saída de lote vinculada.");
      return;
    }

    const key = context.getProperty("Key") as string;
    const sequence = context.getProperty("Sequence") as number;
    ...
    try {
      // O lote do transbordo vem do romaneio de ENTRADA: EntryStorageTransactionKey aponta um
      // Receipt que carrega o StorageAddressCode. Entidade lida por bindContext + requestObject
      // (sem $select), mesmo caminho de Attach.controller#routeMatched — não relativo ao
      // binding da tabela, então o autoExpandSelect não tem bindings de controle pra derivar o
      // $select e devolve a entidade inteira.
      const entryReceipt = await (this.getModel() as ODataModel)
        .bindContext(`/StorageTransactions(${entryStorageTransactionKey})`)
        .requestObject() as { StorageAddressCode?: string };

      const lotExits = await this.loadEligibleLotExitOptionsAsync(entryReceipt.StorageAddressCode ?? "");

      if (lotExits.length === 0) {
        MessageBox.alert(
          "Não há romaneio de Saída de Armazenagem confirmado, sem vínculo, para o lote deste " +
          "transbordo. Pese a saída antes de vincular.");
        return;
      }

      this.viewModel().setProperty("/transshipmentLotExitDialog", {
        title: `Vincular Saída do Lote — ${formatter.formatTransshipmentLabel(
          { Sequence: sequence, WarehouseCode: warehouseCode, WarehouseName: warehouseName })}`,
        key,
        lotExitKey: null,
        lotExits,
      } as TransshipmentLotExitForm);

      await this.openTransshipmentLotExitDialog();
    } catch (e) {
      MessageBox.error((e as Error).message || "Erro ao preparar a vinculação da saída do lote.");
    } finally {
      this.setBusy(false);
    }
  }
  ...
  /**
   * O fragmento é carregado com o id da VIEW, como o diálogo de Iniciar/Registrar Entrada:
   * `addDependent` DENTRO do `if`, senão cada abertura re-insere o mesmo controle na agregação de
   * dependentes.
   */
  private async openTransshipmentLotExitDialog(): Promise<void> {
    if (!this._transshipmentLotExitDialog) {
      this._transshipmentLotExitDialog = await Fragment.load({
        id: this.getView().getId(),
        name: "siagrob1.view.shipmentLoads.fragments.ShipmentLoadTransshipmentLotExitDialog",
        controller: this,
      }) as Dialog;

      this.getView().addDependent(this._transshipmentLotExitDialog);
    }

    this._transshipmentLotExitDialog.open();
  }

  onCloseTransshipmentLotExitDialog(): void {
    this._transshipmentLotExitDialog?.close();
  }

  async onConfirmTransshipmentLotExitDialog(): Promise<void> {
    // Trava ANTES do primeiro await: duplo clique dispararia duas actions.
    if (this._transshipmentLotExitInFlight) return;

    const form = this.viewModel().getProperty("/transshipmentLotExitDialog") as TransshipmentLotExitForm;

    if (!form.lotExitKey) {
      MessageBox.alert("Selecione o romaneio de saída do lote.");
      return;
    }

    this._transshipmentLotExitInFlight = true;
    this._transshipmentLotExitDialog?.setBusy(true);
    this.setBusy(true);

    try {
      const action = (this.getModel() as ODataModel)
        .bindContext("/ShipmentLoadsTransshipmentAttachLotExit(...)");
      action.setParameter("Key", form.key);
      action.setParameter("LotExitStorageTransactionKey", form.lotExitKey);
      await action.invoke();

      MessageToast.show("Saída do lote vinculada ao transbordo.");

      // O diálogo só fecha DEPOIS do resolve: fechar antes descartaria a seleção se a action
      // recusasse o vínculo. `refresh` no contexto do elemento e nas tabelas — sem isso a tela
      // mostra o estado velho (Situação continuaria "Aguardando saída").
      this.onCloseTransshipmentLotExitDialog();
      this.refreshTransshipments();
    } catch (e) {
      MessageBox.error((e as Error).message || "Erro ao vincular a saída do lote.");
    } finally {
      this._transshipmentLotExitInFlight = false;
      this._transshipmentLotExitDialog?.setBusy(false);
      this.setBusy(false);
    }
  }
