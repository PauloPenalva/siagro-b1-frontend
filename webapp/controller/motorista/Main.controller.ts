...
export default class Main extends BaseController {
  formatter = { ...formatter }
	...
	onInit(): void | undefined {
		this.getRouter().getRoute("motoristas").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void | undefined {
		(this.getView().byId("tableMotorista").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void | undefined {
		...
		(this.getView().byId("tableMotorista").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
			this.navTo("motoristasNew");
	}

	onEdit(): void {
		const oTable = this.byId("tableMotorista") as Table;
