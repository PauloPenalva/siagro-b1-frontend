...
export default class Main extends BaseController {
  ...
  onInit(): void {
    this.getRouter().getRoute("notificationGroups")
  ...
  onRefresh(): void {
    (this.getView().byId("tableNotificationGroups").getBinding("rows") as ODataListBinding).refresh();
  }
  ...
  onSearch(ev: SearchField$SearchEvent): void {
    ...
    (this.getView().byId("tableNotificationGroups").getBinding("rows") as ODataListBinding)
      .filter([oFilters]);
  }

  onCreate() {
    this.navTo("notificationGroupsNew");
  }

  onEdit(): void {
    const oTable = this.byId("tableNotificationGroups") as Table;
    ...
    this.navTo("notificationGroupsEdit", { id: sId });
  }
