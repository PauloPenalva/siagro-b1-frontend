...
export default class Main extends BaseController {
	...
	onInit(): void {
		this.getRouter().getRoute("roles").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void {
		(this.getView().byId("tableRoles").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void {
		...
		(this.getView().byId("tableRoles").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}
	...
	onEdit(): void {
		const oTable = this.byId("tableRoles") as Table;
