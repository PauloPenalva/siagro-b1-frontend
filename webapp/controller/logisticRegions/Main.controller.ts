...
export default class Main extends BaseController {
  formatter = { ...formatter }
	...
	onInit(): void | undefined {
		this.getRouter().getRoute("logisticRegions").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void | undefined {
		(this.getView().byId("tableLogisticRegions").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void | undefined {
		...
		(this.getView().byId("tableLogisticRegions").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
			this.navTo("logisticRegionsNew");
	}

	onEdit(): void {
		const oTable = this.byId("tableLogisticRegions") as Table;
    ...
    if (i < 0){
      ...
      return;
		...
		this.navTo("logisticRegionsEdit", {id: sId});
	}
