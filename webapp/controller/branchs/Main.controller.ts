...
export default class Main extends BaseController {
  formatter = { ...formatter }
	...
	onInit(): void | undefined {
		this.getRouter().getRoute("branchs").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void | undefined {
		(this.getView().byId("tableBranchs").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void | undefined {
		...
		const oFilters = new Filter({
			filters: [
				new Filter("BranchName", FilterOperator.Contains, query),
				new Filter("Code", FilterOperator.Contains, query),
			],
			and: false,
		});

		(this.getView().byId("tableBranchs").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
			this.navTo("branchsNew");
	}

	onEdit(): void {
		const oTable = this.byId("tableBranchs") as Table;
