...
/**
 * @namespace siagrob1.controller.profiles
 */
export default class Main extends BaseController {
	...
	onSearch(ev: SearchField$SearchEvent): void {
		...
		(this.getView().byId("tableProfiles").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
			this.navTo("profilesNew");
	}

	onEdit(): void {
		const oTable = this.byId("tableProfiles") as Table;
    ...
    if (i < 0){
      ...
      return;
		...
		this.navTo("profilesEdit", {id: sId});
	}
