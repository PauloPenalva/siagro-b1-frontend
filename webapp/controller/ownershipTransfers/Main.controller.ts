...
export default class Main extends CommonController {
  ...
  onCreate() {
		this.navTo("ownershipTransfersNew");
	}

	onDetail(): void {
		const oTable = this.byId("ownershipTransfersTable") as Table;
    ...
    const oContext = oTable.getContextByIndex(i)
		...
		this.navTo("ownershipTransfersDetail", {id: sId});
	}
  ...
  async onCopy() {
    ...
    const bConfirm = await confirmDialog("Copiar Transferencia ?");
    if (bConfirm) {
