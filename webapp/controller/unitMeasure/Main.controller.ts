...
/**
 * @namespace siagrob1.controller.unitMeasure
 */
export default class Main extends BaseController {
  formatter = { ...formatter }
  
	onInit(): void | undefined {
		this.getRouter().getRoute("unitMeasure").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void | undefined {
		(this.getView().byId("unitMeasureTable").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void | undefined {
		...
		(this.getView().byId("unitMeasureTable").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
			this.navTo("unitMeasureNew");
	}

	onEdit(): void {
		const oTable = this.byId("unitMeasureTable") as Table;
    ...
    if (i < 0){
      ...
      return;
		...
		this.navTo("unitMeasureEdit", {id: sId});
	}
