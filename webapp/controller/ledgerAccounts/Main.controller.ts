...
export default class Main extends BaseController {
  formatter = { ...formatter }
	...
	onInit(): void {
		this.getRouter().getRoute("ledgerAccounts").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void {
		(this.getView().byId("ledgerAccountsTable").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void {
		...
		(this.getView().byId("ledgerAccountsTable").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
		this.navTo("ledgerAccountsNew");
	}

	onEdit(): void {
		const oTable = this.byId("ledgerAccountsTable") as Table;
    ...
    if (i < 0){
      ...
      return;
		...
		this.navTo("ledgerAccountsEdit", {id: sId});
	}
