...
/**
 * @namespace siagrob1.controller.qualityAttribs
 */
export default class Main extends BaseController {
  formatter = { ...formatter }
  
	onInit(): void | undefined {
		this.getRouter().getRoute("qualityAttribs").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void | undefined {
		(this.getView().byId("qualityAttribsTable").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void | undefined {
		...
		(this.getView().byId("qualityAttribsTable").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
			this.navTo("qualityAttribsNew");
	}

	onEdit(): void {
		const oTable = this.byId("qualityAttribsTable") as Table;
    ...
    if (i < 0){
      ...
      return;
		...
		this.navTo("qualityAttribsEdit", {id: sId});
	}
