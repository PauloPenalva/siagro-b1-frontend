 ...
 */
export default class Upload extends PurchaseContractsBaseController {

  file: File;

	onInit(): void  {	
		this.getRouter().getRoute("purchaseContractsUpload").attachPatternMatched((ev) => this.editRouteMatched(ev));
	}

	private editRouteMatched(ev: Route$MatchedEvent) {
    const description = this.byId("description") as Input;
    const fileUpload = this.byId('fileUploader') as FileUploader;
  ...
  handleValueChange(ev: FileUploader$ChangeEvent){
    ...
    reader.onload = async () => {
        try {
          ...
          const response = await fetch(`odata/PurchaseContractsAttachmentUpload`,
              {
                  method: "POST",
                  headers: {
