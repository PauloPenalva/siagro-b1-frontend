...
/**
 * Fila da diretoria: fixações de preço aguardando aprovação, de todos os
 * contratos a fixar (PAF).
 *
 * @namespace siagrob1.controller.purchaseContracts.priceFixationApproval
 */
export default class Main extends CommonController {
  ...
  onInit(): void {
    this.getRouter().getRoute("purchaseContractsPriceFixationApproval")
  ...
  /**
   * Abre o contrato de compra da fixação selecionada, para o aprovador conferir os
   * termos antes de decidir. A lista já traz PurchaseContract via $expand, então a
   * chave sai do próprio contexto — sem requisição extra.
   */
  /**
   * Abre o diálogo somente-leitura com todos os dados da fixação selecionada —
   * inclusive vencimento financeiro e dados para pagamento, que o aprovador precisa
   * ver para decidir. Reusa o fragmento da tela de contrato.
   */
  ...
  async onConfirmPriceFixationAction(): Promise<void> {
    ...
    const route = isApprove
      ? this.api.purchaseContractsPriceFixationApproval
      : this.api.purchaseContractsPriceFixationReject;

    this.onClosePriceFixationApprovalDialog();
    this.setBusy(true);

    // bindContext + invoke em vez de jQuery.ajax: a action roda pelo ODataModel.
    // Aprovar/rejeitar tira a fixação da fila (deixa de ser InApproval), então em
    // seguida recarregamos a lista para a linha sumir sem recarregar a rota.
  ...
  /**
   * Monta o resumo do que está sendo decidido. A diretoria aprova um valor
   * financeiro, então volume e preço isolados não bastam — mostra o total.
   */
  private async openActionDialog(action: "Approve" | "Reject"): Promise<void> {
    ...
    // getProperty devolve Edm.Decimal como STRING ("10000.000"), não number.
    // Sem o Number() explícito, toLocaleString não formata nada e a diretoria lê
    // "10000.000 kg × 2.70000000" na hora de decidir.
    ...
    this._priceFixationApprovalDialog ??= await DialogHelper.createDialog(
      this,
      "siagrob1.view.purchaseContracts.priceFixationApproval.fragments.PriceFixationApprovalDialog"
    );
