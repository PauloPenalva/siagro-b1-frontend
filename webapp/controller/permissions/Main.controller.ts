...
/**
 * @namespace siagrob1.controller.permissions
 */
export default class Main extends BaseController {

	onInit(): void {
		this.getRouter().getRoute("permissions").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void {
		(this.getView().byId("tablePermission").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void {
		...
		(this.getView().byId("tablePermission").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
			this.navTo("permissionsNew");
	}

	onEdit(): void {
		const oTable = this.byId("tablePermission") as Table;
    ...
    if (i < 0){
      ...
      return;
		...
		this.navTo("permissionsEdit", {id: sId});
	}
