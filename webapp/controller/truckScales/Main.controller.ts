...
/**
 * @namespace siagrob1.controller.truckScales
 */
export default class Main extends BaseController {
  formatter = { ...formatter }
  
	onInit(): void | undefined {
		this.getRouter().getRoute("truckScales").attachPatternMatched(() => this.routeMatched())
	}
	...
	onRefresh(): void | undefined {
		(this.getView().byId("tableTruckScales").getBinding("rows") as ODataListBinding).refresh();
	}
	...
	onSearch(ev: SearchField$SearchEvent): void | undefined {
		...
		const oFilters = new Filter({
			filters: [
				...
				new Filter("Localization", FilterOperator.Contains, query),
			],
			and: false,
		});

		(this.getView().byId("tableTruckScales").getBinding("rows") as ODataListBinding).filter([oFilters]);
	}

	onCreate() {
			this.navTo("truckScalesNew");
	}

	onEdit(): void {
		const oTable = this.byId("tableTruckScales") as Table;
    ...
    if (i < 0){
      ...
      return;
		...
		this.navTo("truckScalesEdit", {id: sId});
	}
